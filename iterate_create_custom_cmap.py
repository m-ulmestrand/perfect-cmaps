...
def project_onto_gamut(x, y, ab_slice):
...
def onclick(event):
    if event.inaxes == ax:
        global clicked_points, current_L_index
        ...
        ab_slice = occupancy[l_bin_index]
        x, y = project_onto_gamut(x, y, ab_slice)
...
args = parse_args()
L_values = get_lightness_profile(args.num_points, args.lightness)
